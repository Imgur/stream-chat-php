...
$SNIPPETS_MAP['autoload'] = count($SNIPPETS);
$SNIPPETS[] = 'require_once "./vendor/autoload.php";';

$SNIPPETS_MAP['get_client'] = count($SNIPPETS);
$SNIPPETS[] = '$client = new GetStream\StreamChat\Client(getenv("STREAM_API_KEY"), getenv("STREAM_API_SECRET"));';

$SNIPPETS_MAP['get_token'] = count($SNIPPETS);
$SNIPPETS[] = <<<'SNIP'
...
$SNIPPETS_MAP['users'] = count($SNIPPETS);
$SNIPPETS[] = <<<'SNIP'
$bob = [
    'id' => 'bob-1',
    'role' => 'admin',
...
$SNIPPETS_MAP['channeltypes'] = count($SNIPPETS);
$SNIPPETS[] = <<<'SNIP'
...
SNIP;
$SNIPPETS_MAP['channels'] = count($SNIPPETS);
$SNIPPETS[] = <<<'SNIP'
...
$SNIPPETS_MAP['devices'] = count($SNIPPETS);
$SNIPPETS[] = <<<'SNIP'
...
function snippet($name, $run=true, $show=true){
    global $SNIPPETS;
    global $SNIPPETS_MAP;
    $idx = $SNIPPETS_MAP[$name];
    $snippet = $SNIPPETS[$idx];
    // load libs and create a client and append the snippet
    $code = implode(array_slice($SNIPPETS, 0, 2), "\n") . $snippet;
    // make sure it actually works
    if($run){
      eval($code);
    }
    if($show){
        $snippet = str_replace("// echo", "echo", $snippet);
        echo("\n```php\n" . $snippet . "\n```\n");
    }
}
...
### Getting started
<?php snippet('autoload') ?>

Instantiate a new client, find your API keys in the dashboard.
<?php snippet('get_client') ?>

Generate a token for clientside use
<?php snippet('get_token') ?>

## Update / Create users
<?php snippet('users') ?>

## ChannelType CRUD
<?php snippet('channeltypes') ?>

## Channels and messages
<?php snippet('channels') ?>
